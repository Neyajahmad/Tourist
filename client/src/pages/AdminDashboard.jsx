...
const AdminDashboard = () => {
  ...
  return (
    <div className="dashboard-container" style={{ flexDirection: isMobile ? 'column' : 'row' }}>
      ...
      <div className="sidebar sidebar-admin" style={{ display: (!isMobile || activeTab === 'users' || activeTab === 'alerts') ? 'flex' : 'none', flexDirection: 'column' }}>
        {!isMobile && (
              ...
              return (
                <div key={user._id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    {isActive && (
                      <>
                        <button onClick={() => { setFocusUserId(user._id); if (isMobile) setActiveTab('map'); }} style={{ padding: '6px 10px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(59,130,246,0.2)', color: 'white', borderRadius: '8px', cursor: 'pointer' }}>Live</button>
                        <button onClick={() => { setFocusUserId(user._id); setSelectedAlert(null); if (isMobile) setActiveTab('alerts'); }} style={{ padding: '6px 10px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(16,185,129,0.2)', color: 'white', borderRadius: '8px', cursor: 'pointer' }}>Text</button>
      ...
      {/* Main Map Area */}
      <div className="map-wrapper" style={{ display: (!isMobile || activeTab === 'map') ? 'flex' : 'none', flex: 1, position: 'relative', height: isMobile ? 'calc(100vh - 100px)' : '100%', minHeight: 0 }}>
        <MapContainer center={[51.505, -0.09]} zoom={13} style={{ height: '100%', width: '100%' }}>
          ...
          <MapUpdater selectedAlert={selectedAlert} focusUser={focusUserId ? userLocations[focusUserId] : null} activeTab={activeTab} />
        </MapContainer>
...
function MapUpdater({ selectedAlert, focusUser, activeTab }) {
  const map = useMap();
  useEffect(() => {
    if (selectedAlert) {
